...
@if(!($isPdf ?? false))
    ...
    <div class="action-bar no-print">
        <div>
            <a href="{{ route('maintenance.index') }}" class="action-btn secondary">
        ...
        <div style="display: flex; gap: 8px;">
            ...
            @endphp
            @if($subscription->id)
                <form id="formSendWaMaintenance" action="{{ route('invoices.maintenance.send-wa', $subscription->id) }}" method="POST" style="display: inline; margin: 0;">
                    @csrf
                    <button type="button" class="action-btn" onclick="Swal.fire({title:'Konfirmasi',text:'Kirim dokumen PDF Invoice Maintenance ini langsung ke nomor WhatsApp {{ $lead?->kontak_wa }} via Flustra Gateway?',icon:'question',showCancelButton:true,confirmButtonText:'Ya, Kirim',cancelButtonText:'Batal',confirmButtonColor:'#7a8a60',cancelButtonColor:'#71717a',reverseButtons:true}).then(r=>{if(r.isConfirmed)document.getElementById('formSendWaMaintenance').submit()})">
            ...
            <a href="{{ route('invoices.maintenance', ['subscription' => $subscription->id ?? 1, 'format' => 'pdf']) }}" class="action-btn secondary">
                Download PDF
            </a>
            <a href="{{ route('invoices.maintenance', ['subscription' => $subscription->id ?? 1, 'format' => 'word']) }}" class="action-btn secondary">
                Download Word
            </a>
                        ...
                        <tr>
                            ...
                            <td style="vertical-align: top; padding-left: 8px;">
                                ...
                                <div style="font-size: 7.5pt; color: #555555; margin-top: 6px; line-height: 1.3;">
                                    Konfirmasi bukti perpanjangan dapat dikirimkan via WhatsApp resmi atau melalui Portal Klien.
                                </div>
                            </td>
                        </tr>
                    </table>
        ...
        <tr>
            <td style="width: 60%; vertical-align: bottom;">
                <div class="footer-note">
                    ...
                    1. Pembayaran resmi hanya sah melalui rekening {{ $settings->bank_name ?? 'BCA' }} atas nama <strong>{{ $settings->bank_account_holder ?? 'MUHAMAD RYAN RIZKI' }}</strong> atau <strong>QRIS resmi</strong>.<br>
